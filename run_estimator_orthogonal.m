...
%%tolerance  = PowerFlowInputData.tolerance;
...
IPmeasbus=Imeasbus;  
IQmeasbus=Imeasbus;  
%Give ramdom value to TJ and J, ank make sure J>TJ, so that we can enter
...
while J>TJ
   ...
   while (iteration<maxiter)
       ...
       Ro=zeros(nummeas);
       for i=1:numFPmeas
            Ro(i,i)=FmeasPsigma(i);
       end
       for i=(numFPmeas+1):(numFPmeas+numFQmeas)
            Ro(i,i)=FmeasQsigma(i-numFPmeas);
       end
       for i=(numFPmeas+numFQmeas+1):(numFPmeas+numFQmeas+numIPmeas)
            Ro(i,i)=ImeasPsigma(i-numFPmeas-numFQmeas);
       end
       for i=(numFPmeas+numFQmeas+numIPmeas+1):(numFPmeas+numFQmeas+numIPmeas+numIQmeas)
            Ro(i,i)=ImeasQsigma(i-numFPmeas-numFQmeas-numIPmeas);
       end
